...
class MarketCapabilityClient(context: Context) {
    ...
    data class MarketCapability(
        ...
        val message: String?,
        val contractsReady: Boolean = false,
        val metadataReady: Boolean = false,
        val quoteReady: Boolean = false,
        ...
        internal fun parse(root: JSONObject): Snapshot {
            ...
            listOf("BIST","VIOP","COMMODITY","FX","INDEX").forEach { key ->
                ...
                markets[key]=MarketCapability(
                    ...
                    contractsReady=x.optBoolean("contractsReady",false),
                    metadataReady=x.optBoolean("metadataReady",false),
                    quoteReady=x.optBoolean("quoteReady",false),
                    historyReady=x.optBoolean("historyReady",false),
                    liquidityReady=x.optBoolean("liquidityReady",false),
                    freshnessReady=x.optBoolean("freshnessReady",false),
                    scannerReady=x.optBoolean("scannerReady",false)
                )
            }
